...
export default class Folders2GraphPlugin extends Plugin {
	...
	private graphSidebarControls!: GraphSidebarControls;
	...
	public override async onload(): Promise<void> {
		...
		this.graphSidebarControls = new GraphSidebarControls(this);
	}
	...
	public refreshGraphLeaves(leaves: GraphLeafWithCustomRenderer[] = this.__getLeavesOfTypeGraph()): void {
		leaves.forEach((leaf) => {
			if (leaf.view.getViewType() === "graph") {
				...
				leaf.view.load();
				setTimeout(() => {
					this.graphSidebarControls.inject(leaf);
				}, 0);
			}
		});
	}
	...
	private __injectDataInLeaf(leaf: GraphLeafWithCustomRenderer): void {
		...
		renderer.setData = (data: RendererData) => {
			...
			if (this.settings.HiddenNodes) {
				const hiddenStrings = this.settings.HiddenNodesString
					.split(" ")
					.map((s) => s.trim())
					.filter(Boolean);
